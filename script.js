...
async function processarArquivoCSV(arquivo) {
    ...
    // Verificar duplicatas por chave E por conteúdo (mercado+dataLimpa+valor)
    const chavesConteudoExistentes = new Set(
        snapExistentes.docs.map(d => {
            const op = d.data();
            const dataLimpa = op.dataHora ? normalizarDataBetfair(op.dataHora).split(' ')[0] : '';
            const mercadoNorm = (op.mercado || '').toLowerCase().trim();
            const lucroInt = Math.round((op.lucro || 0) * 100);
            return `${mercadoNorm}|${dataLimpa}|${lucroInt}`;
        })
    );

    const novas = operacoesArquivo.filter(op => {
        if (chavesExistentes.has(op.chave)) return false;
        const dataLimpa = op.dataHora ? op.dataHora.split(' ')[0] : '';
        const mercadoNorm = op.mercado.toLowerCase().trim();
        const lucroInt = Math.round((op.lucro || 0) * 100);
        const chaveConteudo = `${mercadoNorm}|${dataLimpa}|${lucroInt}`;
        return !chavesConteudoExistentes.has(chaveConteudo);
    });
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (!usuarioAtual) return;
    if (!confirm('🧹 Deduplicar dados do Firestore?\n\nDetecta duplicatas por CONTEÚDO (mercado + dia + valor), capturando duplicatas entre diferentes formatos de CSV.\n\nSeus dados únicos serão preservados.')) return;
    ...
    try {
        ...
        snap.docs.forEach(d => {
            const op = d.data();
            const dataLimpa = op.dataHora ? normalizarDataBetfair(op.dataHora).split(' ')[0] : (op.dataLimpa || '');
            const mercadoNorm = (op.mercado || '').toLowerCase().trim();
            const lucroInt = Math.round((op.lucro || 0) * 100);
            const chaveConteudo = `${mercadoNorm}|${dataLimpa}|${lucroInt}`;
            const chaveId = op.chave && op.chave.startsWith('betId:') ? op.chave : null;

            if (chaveId) {
                if (vistos.has(chaveId) || vistos.has(chaveConteudo)) {
                    paraExcluir.push(d.id);
                } else {
                    vistos.set(chaveId, d.id);
                    vistos.set(chaveConteudo, d.id);
                }
            } else {
                if (vistos.has(chaveConteudo)) {
                    paraExcluir.push(d.id);
                } else {
                    vistos.set(chaveConteudo, d.id);
                }
